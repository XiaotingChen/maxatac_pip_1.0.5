    ...
    if input_seq is not None:
        ...
        if INTER_FUSION:
            ...
            for i, input_sample in enumerate(input_samples):
                genome, signal = input_sample["genome"], input_sample["atac"]       # Shape = (1, seq_len, 4) and (1, seq_len, 1)
                contribs = []
    ...
    if input_seq is None:
        ...
        for j in range(min(max_num_samples, relevant_inputs.shape[0])):
            ...
            if visualize:
                ...
                plot_ism_results(difference, save_dir=f"{output_full_dir}/ism-{cell_type}-{chromosome}-{npeaks}peaks-id{j}.png")
        ...
        for j in range(min(num_input_seqs, max_num_samples)):
            ...
            if visualize:
                ...
                save_dirs = [f"{output_full_dir}/ism-{cell_type}-{chromosome}-bases{'_'.join(b)}-npeaks{npeaks}-id{j}.png" for b in l]
                os.makedirs(output_full_dir, exist_ok=True)
...
def check_attention_offset_distribution(meta_file, chromosome, cell_type, output_dir, model_base_dir):
    ...
    for sub_model in sub_models:
        ...
        if len(outputs) == 2:
            ...
            _, weights, _ = outputs
        base_highest_attention = np.argmax(weights, axis=2)     # shape = (batch_size, 4, 512)
        base_highest_att_med = np.median(base_highest_attention, axis=0)    # shape = (4, 512)
        dist = np.repeat(
            np.expand_dims(np.arange(base_highest_att_med.shape[1], dtype=float), axis=0), repeats=base_highest_att_med.shape[0], axis=0
    ...
    for i, base_att in enumerate(base_attentions):
        for head in range(base_att.shape[0]):
            axes[i].plot(x, base_att[head], "o", label=f"Head {head}", color=colors[head])
...
def check_layer_contributions(meta_file, chromosome, cell_type, output_dir, model_base_dir, max_num_seqs=15, npeaks=9, mode="b"):
    ...
    last_downsampling_conv_layer = sorted([layer.name for layer in model.layers if "downsampling_conv" in layer.name])[-1]
    relevant_layers.append(last_downsampling_conv_layer)

    # Get the part of the model after the last transformer layer
    output_model = tf.keras.Model(model.get_layer("conv1d_1").input, model.layers[-1].output)
    ...
    if mode == "b":
        ...
        transformer_layers = [l for l in relevant_layers if "downsampling_conv" not in l]
        for i in range(len(transformer_layers)):
            ...
            first_model = tf.keras.Model(model.input, model.get_layer(first_output).output)
            last_model = tf.keras.Model(model.get_layer(last_input).input, model.layers[-1].output)
            temp = first_model.predict(relevant_inputs)
            if type(temp) == tuple: temp = temp[0]
...
def plot_attention_weights(
    ...
    for s in range(min(len(input_sample), max_num_samples)):
        ...
        for l in range(num_layers):
            ...
            # Make an inference
            outputs = submodel.predict(input_sequence, verbose=0) # att_weights.shape = (num_head, height, width)
            if len(outputs) == 2:
        ...
        att_matrix = np.stack(att_layers, axis=0)       #shape = (num_layer, num_head, height, width)
        if z_score: att_matrix = zscore(att_matrix)
    ...
    if visualize:
        ...
        for s, att_matrix in enumerate(att_matrices):
            ...
            fig, axes = plt.subplots(nrows=num_layers+1, ncols=num_heads+1, figsize=(40, 40))
            for l in range(num_layers):
                for h in range(num_heads):

                    sns.heatmap(data=att_matrix[l, h, :, :], ax=axes[l][h], cmap=palette, center=0)
                ...
                sns.heatmap(data=agg_weights_across_heads, ax=axes[l][num_heads], cmap=palette, center=0)
                ...
                sns.heatmap(data=agg_weights_across_layers, ax=axes[num_layers][l], cmap=palette, center=0)
            ...
            sns.heatmap(data=agg_all_weights, ax=axes[num_layers][num_heads], cmap=palette, center=0)
...
def plot_latent_signal_vector(meta_file, chromosome, cell_type, output_dir, model_base_dir, input_seq=None, visualize=False, max_num_samples=15, npeaks=9):
    ...
    if input_seq is not None:
        ...
        for layer in layers_before_transformer:
            layer_output = tf.keras.Model(model.inputs, model.get_layer(layer).output)(input_sample) # Shape = (num_samples, seq_len, feature_dim)
            # Very hard-coded here
            if layer_output.shape[1] != 256:
                if "fusion" in layer: latent_vectors.append(layer_output[:, 256:, :])
                elif "maxpool" in layer:
                    latent_vectors.append(
                        np.amax(
                            np.reshape(layer_output, (layer_output.shape[0], layer_output.shape[1] // 2, 2, layer_output.shape[2])),
                            axis=2)
                    )
            else:
        ...
        for t in transformer_names:
            _, _, transformer_latent_vectors = tf.keras.Model(model.inputs, model.get_layer(t).output)(input_sample)
...
def plot_latent_genome_vector(meta_file, chromosome, cell_type, output_dir, model_base_dir, input_seq=None, visualize=False, max_num_samples=15, npeaks=9):
    ...
    if input_seq is not None:
        ...
        for t in transformer_names:
            _, _, transformer_latent_vectors = tf.keras.Model(model.inputs, model.get_layer(t).output)(input_sample)
            latent_vectors.append(transformer_latent_vectors[0][:, :256, :])        # Hard-coded
...
def ism_attention_weights_correlation(
    ...
    def plot_att_row(att_row, ax):
        ...
        yticks_label = [f"L{l}-H{h}" for l in range(0, num_layer) for h in range(0, num_head, 2)]
    ...
    inputs, targets, moods = get_data(meta_file, chromosome, cell_type, output_dir, moods_bigwig=moods_bigwig)
    ...
    output_seq = np.squeeze(relevant_outputs[:max_num_samples, :])       # shape = (num_samples, 32)
    ...
    for sample in range(output_seq.shape[0]):
        ...
        if avg not in [13, 14, 15, 16]: bases_of_interest.append([avg])
    ...
    output_seq = np.repeat(output_seq, repeats=((att_matrices[0].shape[-1]//2) // output_seq.shape[-1]), axis=1)

    assert len(ism_results) == len(att_matrices), f"The number of attention matrices={len(att_matrices)} and ISM={len(ism_results)} results are not equal!"
    for s in range(min(max_num_samples, relevant_inputs.shape[0])):
        ...
        num_mha, num_head, att_weight_width = att_matrix.shape[0], att_matrix.shape[1], att_matrix.shape[-1]

        # Reshape input ATAC-seq signal
        signal = relevant_inputs[s, :, 4:5]
        signal_resized = np.amax(np.reshape(np.amax(signal, axis=1), (-1, (input_length // att_weight_width)*2)), axis=-1)

        # Reshape ISM vector
        ism_resized = np.amax(np.reshape(np.amax(ism_result[0], axis=1), (-1, (input_length // att_weight_width)*2)), axis=-1)
        ...
        if INTER_FUSION:
            ...
            for i in range(atac_latent_vector.shape[0]):
                latent_atac = atac_latent_vector[i, ...]        # shape = (seq_len, feature_dims)
                ...
                axes[i+3].set_title(f"Latent ATAC-seq vector after {all_layer_names[i]}")

            # Plot the genome latent signal (axes 9-14)
            for i in range(genome_latent_vector.shape[0]):
                latent_genome = genome_latent_vector[i, ...]        # shape = (seq_len, feature_dims)
                ...
                axes[i+9].set_title(f"Latent genome vector after {all_layer_names[i]}")

            # Plot ISM (axes 15)
            l = [[str(y) for y in z] for z in bases_of_interest_list[s]]
            for k, ism in enumerate(ism_result):
                ism_resized = np.amax(np.reshape(np.amax(ism, axis=1), (-1, (input_length // att_weight_width)*2)), axis=-1)
            ...
            for k, shap in enumerate(shap_atac_result):
                ...
                shap_resized = np.amax(np.reshape(shap, (-1, (input_length // att_weight_width)*2)), axis=-1)
            ...
            for k, shap in enumerate(shap_genome_result):
                ...
                shap_resized = np.amax(np.reshape(np.amax(np.squeeze(shap), axis=1), (-1, (input_length // att_weight_width)*2)), axis=-1)
            ...
            #plt.tight_layout()
            plt.savefig(f"{output_dir}/z-ismmat-{model_name}-{cell_type}-npeaks{npeaks}-id{s}.png")
                    

def input_maximization(meta_file, chromosome, cell_type, output_dir, model_base_dir, npeaks=9, max_num_samples=2):
    """
