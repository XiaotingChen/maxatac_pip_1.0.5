...
def run_normalization(args):
    ...
    if args.max:
        ...
        min_value, max_value, median, mad, mean_value, std_value = get_genomic_stats(
            bigwig_path=args.signal,
            chrom_sizes_dict=chromosome_length_dictionary,
            blacklist_path=args.blacklist,
            max_percentile=args.max_percentile,
            prefix=os.path.join(args.output, args.prefix),
        )
        ...
        logging.error(
            ...
            + str(median)
            + "\n  Genomic median absolute deviation (non-zero): "
            + str(mad)
            + "\n  Genomic mean: "
            + str(mean_value)
    ...
    with pyBigWig.open(args.signal) as input_bw, pyBigWig.open(
        ...
        for chrom_name, chrom_length in header:
            ...
            blacklist_mask = chromosome_blacklist_mask(
                args.blacklist, chrom_name, chromosome_length_dictionary[chrom_name]
            )
