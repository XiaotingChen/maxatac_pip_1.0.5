...
class TransformerBlock(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        return (
            x_mlp5 + mha_output,
            att_weights,
            (x1, x2, x3, x_mlp1, x_mlp2, x_mlp3, x_mlp4, x_mlp5),
        )
...
class TransformerBlockCrossAtt(TransformerBlock):
    ...
    def build(self, input_shape):
        # self.mha_ln = snt.LayerNorm(axis=-1, create_scale=True, create_offset=True)
        self.mha_ln_kv = tf.keras.layers.LayerNormalization(
            axis=-1, scale=True, center=True
        )
        self.mha_ln_q = tf.keras.layers.LayerNormalization(
            axis=-1, scale=True, center=True
        )
...
class MultiheadAttention(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self._embedding_layer = tf.keras.layers.Dense(
            embedding_size, name="embedding_layer", kernel_initializer=w_init
        )
        ...
        if self._relative_positions:
            ...
            try:
                ...
                temp = self._initializer(
                    [self._num_heads, self._key_size], dtype=tf.float32
                )
                temp = tf.reshape(temp, (1, self._num_heads, 1, self._key_size))
                self._r_w_bias = tf.Variable(temp, name="r_w_bias")
            ...
            try:
                ...
                temp = tf.reshape(temp, (1, self._num_heads, 1, self._key_size))
                self._r_r_bias = tf.Variable(temp, name="r_r_bias")
    ...
    def _multihead_output(self, _layer, inputs, batch_size):
        ...
        output = tf.reshape(
            output, (-1, tf.shape(output)[1], self._num_heads, num_kv_channels)
        )
    ...
    def call(self, inputs, training=False):
        ...
        if self._relative_positions:
            ...
            # [1, H, 2T-1, K]
            r_k = self._multihead_output(
                self._r_k_layer, positional_encodings, batch_size
            )
        ...
        attended_inputs = tf.reshape(output_transpose, (batch_size, -1, embedding_size))
...
class MultiheadCrossAttention(MultiheadAttention):
    ...
    def call(self, key_value_inputs, query_inputs, training=False):
        ...
        q = self._multihead_output(
            self._q_layer, query_inputs, batch_size
        )  # [B, H, T, K]
        k = self._multihead_output(
            self._k_layer, key_value_inputs, batch_size
        )  # [B, H, T, K]
        v = self._multihead_output(
            self._v_layer, key_value_inputs, batch_size
        )  # [B, H, T, V]
...
def positional_features_gamma(
    ...
    outputs = probabilities / tf.reduce_max(probabilities, axis=1, keepdims=True)
    tf.TensorShape(outputs.shape).assert_is_compatible_with(
        positions.shape + [feature_size]
    )
    return outputs
