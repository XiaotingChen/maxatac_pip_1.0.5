...
with Mute():
    ...
    from maxatac.architectures.attention_module_TF import (
        TransformerBlock,
        TransformerBlockCrossAtt,
    )
...
def get_multiinput_crossatt_transformer(
    ...
    signal_layer = get_conv_tower(
        signal_layer,
        model_config["CONV_TOWER_CONFIGS_FUSION"]["signal"],
        model_config["DOWNSAMPLE_METHOD_CONV_TOWER"],
        "signal",
    )
    ...
    new_rpe_crossatt_genome_attention_kwargs = deepcopy(
        model_config["WHOLE_ATTENTION_KWARGS_SELFATT_GENOME"]
    )
    new_rpe_crossatt_genome_attention_kwargs["initializer"] = initializers.get(
        model_config["WHOLE_ATTENTION_KWARGS_SELFATT_GENOME"]["initializer"]
    )
    ...
    new_rpe_crossatt_signal_attention_kwargs = deepcopy(
        model_config["WHOLE_ATTENTION_KWARGS_CROSSATT_SIGNAL"]
    )
    new_rpe_crossatt_signal_attention_kwargs["initializer"] = initializers.get(
        model_config["WHOLE_ATTENTION_KWARGS_CROSSATT_SIGNAL"]["initializer"]
    )
    for i in range(model_config["NUM_MHA_CROSSATT"]):
        ...
        genome_layer, att_weights = transformer_block_crossatt(
            signal_layer, genome_layer
        )

    # Get the token from the sequences
    # signal_layer and genome_layer now has shape (None, seq_len, feature_dim)
    genome_token = tf.keras.layers.Lambda(
        lambda x: x[:, 0, :], name="Extract_signal_token"
    )(
        genome_layer
